...
// Category color mappings - same as ResourceCard
    ...
    <div className="space-y-3">
      {resources.map((resource) => {
        ...
        return (
          <div 
            key={resource.id} 
            className="flex items-center border rounded-lg p-3 transition-all hover:shadow-md bg-white dark:bg-card/80 dark:border-border/40 dark:hover:shadow-primary/10 backdrop-blur-sm group"
          >
            <div className={`p-2 rounded-md ${color} flex items-center justify-center mr-4 h-12 w-12 dark:bg-opacity-90`}>
              {icon}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <h3 className="font-medium line-clamp-1 group-hover:text-primary transition-colors mr-2 cursor-pointer">
                        {resource.title}
                      </h3>
                    </TooltipTrigger>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Badge className={`${getCategoryColorClass(resource.category)} shrink-0`}>
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent side="bottom" align="start">
                    <p className="text-xs">{resource.description || 'No description available'}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
